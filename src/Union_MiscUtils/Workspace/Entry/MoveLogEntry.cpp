namespace NAMESPACE
{
	...
	CInvoke<void(__fastcall*)(oCLogTopic*, zSTRING const&)> Ivk_oCLogTopic_AddEntry(ZenDef<TInstance>(0x0072EA60, 0x0076D0B0, 0x0077A630, 0x00663870), &Hook_oCLogTopic_AddEntry, IvkEnabled(ENGINE));
	void __fastcall Hook_oCLogTopic_AddEntry(oCLogTopic* _this, zSTRING const& a0)
	{
		Ivk_oCLogTopic_AddEntry(_this, a0);

		if (!settings.moveLogEntry)
		{
			Ivk_oCLogTopic_AddEntry.Detach();
			return;
		}
