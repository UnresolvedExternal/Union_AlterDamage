...
namespace NAMESPACE
{
	...
	//CSubscription toggleNearestNpcFocus(TGameEvent::Loop, []()
	//	{
	//		if (!zinput->KeyToggled(KEY_V))
	//			return;
	//		
	//		CVobCollector<oCNpc> collector(ogame->GetGameWorld());
	//		oCNpc* nearestNpc = nullptr;

	//		for (oCNpc* npc : collector.vobs)
	//		{
	//			if (npc == player)
	//				continue;
	//			if (!nearestNpc || player->GetDistanceToVob2(*npc) < player->GetDistanceToVob2(*nearestNpc))
	//				nearestNpc = npc;
	//		}

	//		if (nearestNpc)
	//		{
	//			if (nearestNpc->HasFlag(NPC_FLAG_NFOCUS))
	//				nearestNpc->variousFlags &= ~NPC_FLAG_NFOCUS;
	//			else
	//				nearestNpc->variousFlags |= NPC_FLAG_NFOCUS;
	//			LOG(nearestNpc->variousFlags);
	//			LOG(offsetof(oCNpc, variousFlags));

	//			auto scope = AssignTemp(nearestNpc->variousFlags, nearestNpc->variousFlags);
	//			cmd << "TEST" << endl;
	//			nearestNpc->variousFlags = 12;
	//			LOG(nearestNpc->variousFlags);
	//			nearestNpc->SetFlag(7);
	//			LOG(nearestNpc->variousFlags);
	//		}
	//	}, CHECK_THIS_ENGINE);
}