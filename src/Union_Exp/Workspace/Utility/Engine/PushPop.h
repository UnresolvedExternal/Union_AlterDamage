namespace NAMESPACE
{
	...
	T PopArgument(zCParser* parser)
	{
		...
		{
			CSymbol symbol(parser, parser->datastack.Pop());
...
#define ZPOP(...) PopArguments(zCParser::GetParser(), __VA_ARGS__)
	...
	CSub initParserStrings{ ZSUB(TGameEvent::Execute), []()
		{
			parserStrings = new std::vector<std::unique_ptr<zSTRING>>;
			parserStrings->reserve(16u);
		}
	};

	CSub cleanParserStrings;
	...
	void PushArgument(zCParser* parser, const T& arg, int argSymbol = -1)
	{
		...
		else if constexpr (std::is_convertible_v<T, const char*>)
		{
			if (parserStrings.empty())
				cleanParserStrings = { TGameEvent::Loop | TGameEvent::MenuLoop | TGameEvent::Exit, &CleanParserStrings };
