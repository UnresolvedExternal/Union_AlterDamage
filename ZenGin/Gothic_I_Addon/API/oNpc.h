...
namespace Gothic_I_Addon {
  ...
  const int NPC_TAL_MAX                           = 23;
  const char PLAYER_STEAL_NPC_IS_AWARE[]          = "PLAYER_STEAL_NPC_IS_AWARE";
  const char PLAYER_STEAL_NO_TALENT[]             = "PLAYER_STEAL_NO_TALENT";
  const char PLAYER_STEAL_NOT_IN_RANGE[]          = "PLAYER_STEAL_NOT_IN_RANGE";
  const char PLAYER_STEAL_NPC_IS_EMPTY[]          = "PLAYER_STEAL_NPC_IS_EMPTY";
  const char PLAYER_MOB_MISSING_KEY[]             = "PLAYER_MOB_MISSING_KEY";
  const char PLAYER_MOB_MISSING_LOCKPICK[]        = "PLAYER_MOB_MISSING_LOCKPICK";
  const char PLAYER_MOB_MISSING_KEY_OR_LOCKPICK[] = "PLAYER_MOB_MISSING_KEY_OR_LOCKPICK";
  const char PLAYER_MOB_NEVER_OPEN[]              = "PLAYER_MOB_NEVER_OPEN";
  const char PLAYER_MOB_TOO_FAR_AWAY[]            = "PLAYER_MOB_TOO_FAR_AWAY";
  const char PLAYER_MOB_WRONG_SIDE[]              = "PLAYER_MOB_WRONG_SIDE";
  const char PLAYER_MOB_MISSING_ITEM[]            = "PLAYER_MOB_MISSING_ITEM";
  const char PLAYER_MOB_ANOTHER_IS_USING[]        = "PLAYER_MOB_ANOTHER_IS_USING";
  const char PLAYER_PLUNDER_IS_EMPTY[]            = "PLAYER_PLUNDER_IS_EMPTY";
  const char PLAYER_RANGED_NO_AMMO[]              = "PLAYER_RANGED_NO_AMMO";
  ...
  const int NPC_GIL_ERZBARONE                     = 1;
  const int NPC_GIL_SOELDNER                      = 2;
  const int NPC_GIL_SCHATTEN                      = 3;
  const int NPC_GIL_FEUERKREIS                    = 4;
  const int NPC_GIL_VOLK                          = 5;
  const int NPC_GIL_WASSERKREIS                   = 6;
  const int NPC_GIL_GARDE                         = 7;
  const int NPC_GIL_ORGANISATOREN                 = 8;
  const int NPC_GIL_BAUERN                        = 9;
  const int NPC_GIL_SCHUERFERBUND                 = 10;
  const int NPC_GIL_GURUS                         = 11;
  const int NPC_GIL_NOVIZEN                       = 12;
  const int NPC_GIL_TEMPLER                       = 13;
  const int NPC_GIL_DAEMONEN                      = 14;
  const int NPC_GIL_MEISTER                       = 15;
  const int NPC_GIL_HUMANS                        = 16;
  const int NPC_GIL_WARAN                         = 17;
  const int NPC_GIL_SCHLAEFER                     = 18;
  const int NPC_GIL_GOBBO                         = 19;
  const int NPC_GIL_TROLL                         = 20;
  const int NPC_GIL_SNAPPER                       = 21;
  const int NPC_GIL_MINECRAWLER                   = 22;
  const int NPC_GIL_MEATBUG                       = 23;
  const int NPC_GIL_SCAVANGER                     = 24;
  const int NPC_GIL_DEMON                         = 25;
  const int NPC_GIL_WOLF                          = 26;
  const int NPC_GIL_SHADOW                        = 27;
  const int NPC_GIL_BLOODFLY                      = 28;
  const int NPC_GIL_SHARK                         = 29;
  const int NPC_GIL_ZOMBIE                        = 30;
  const int NPC_GIL_UNDEAD                        = 31;
  const int NPC_GIL_SKELETON                      = 32;
  const int NPC_GIL_ORCDOG                        = 33;
  const int NPC_GIL_MOLERAT                       = 34;
  const int NPC_GIL_GOLEM                         = 35;
  const int NPC_GIL_LURKER                        = 36;
  const int NPC_GIL_ORCS                          = 37;
  const int NPC_GIL_ORCSHAMAN                     = 38;
  const int NPC_GIL_ORCWARRIOR                    = 39;
  const int NPC_GIL_ORCSCOUT                      = 40;
  const int NPC_GIL_ORCSLAVE                      = 41;
  const int NPC_GIL_MAX                           = 42;
